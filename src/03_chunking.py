...
INPUT_KMQ = BASE_DIR / "data" / "processed" / "court_decisions.json"
INPUT_AMENDMENTS = BASE_DIR / "data" / "processed" / "amendments.json"
INPUT_SOURCE_DOCS = BASE_DIR / "data" / "processed" / "source_documents.json"
...
def build_chunks_from_source_documents(source_records):
    """
    Converts source-document (reference) records into embedding-ready chunks.

    These records are archival in nature (e.g. the founding law that approved
    the Code), so they are kept as single, unsplit chunks -- there is no
    clause-level structure to break them down by.

    Args:
        source_records (list[dict]): A list of parsed source-document records.

    Returns:
        list[dict]: A list of chunk dictionaries with appended headers.
    """

    chunks = []
    for i, rec in enumerate(source_records, start=1):
        header = "Mənbə sənəd (Məcəllənin qəbulu/təsdiqi ilə bağlı arayış)"
        chunks.append({
            "chunk_id": f"source_document_{i}",
...
def main():
    ...
    with open(INPUT_AMENDMENTS, encoding="utf-8") as f:
        amendment_records = json.load(f)
    with open(INPUT_SOURCE_DOCS, encoding="utf-8") as f:
    ...
    kmq_chunks = build_chunks_from_kmq(kmq_records)
    amendment_chunks = build_chunks_from_amendments(amendment_records)
    source_chunks = build_chunks_from_source_documents(source_records)
    all_chunks = article_chunks + kmq_chunks + amendment_chunks + source_chunks
